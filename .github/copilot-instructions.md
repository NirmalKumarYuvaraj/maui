...
**⚠️ PR Agent Workflow Reminder:** The `pr` agent has a **mandatory 4-phase workflow** (Pre-Flight → Gate → Fix → Report). When delegating to it, the agent MUST start with Phase 1 (Pre-Flight) regardless of user wording. Even "fix issue" means "start investigating from Phase 1", not "jump to coding a fix".

**Examples of correct delegation**:

- User: "Review PR #12345" → Immediately invoke **pr** agent (starts at Phase 1)
- User: "Fix issue #67890" → Immediately invoke **pr** agent (starts at Phase 1 — NOT at fixing)
